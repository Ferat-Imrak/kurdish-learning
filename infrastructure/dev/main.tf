...
locals {
  ...
  backend_api_url = startswith(local.backend_service_url, "http") ? "${local.backend_service_url}/api" : "https://${local.backend_service_url}/api"
}
...
module "frontend" {
  ...
  build_spec_override       = var.amplify_build_spec_override
}
