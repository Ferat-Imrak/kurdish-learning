...
const getPictureQuizProgress = (deckName: string): { score: number; total: number } | null => {
  ...
  const stored = localStorage.getItem(`picturequiz-progress-${deckName}`)
  return stored ? JSON.parse(stored) : null
}

const savePictureQuizProgress = (deckName: string, score: number, total: number) => {
  if (typeof window === 'undefined') return
  // Save progress when score is 80% or higher
  if (score / total >= 0.8) {
    const existing = localStorage.getItem(`picturequiz-progress-${deckName}`)
    const bestScore = existing ? Math.max(JSON.parse(existing).score, score) : score
    localStorage.setItem(`picturequiz-progress-${deckName}`, JSON.stringify({ score: bestScore, total }))
  }
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-kurdish-red/10 via-white to-kurdish-green/10">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          {!selectedDeck && (
            <h1 className="text-2xl md:text-3xl font-bold text-kurdish-red text-center">Translation Quiz</h1>
          )}
        </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {questions[currentQuestion].options.map((option, index) => (
                              <motion.button
                                ...
                                className="bg-gray-100 hover:bg-kurdish-green hover:text-white p-4 rounded-xl font-bold text-lg transition-colors"
